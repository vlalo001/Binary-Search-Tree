...
struct TreeNode* InsertNewNode(struct TreeNode *MainNode, char *szKey, int iIsCharacterSensitive)
{
	...
	else
	{
		...
		// key is greater than main node key, add to right
...
// recursively prints to stdout OR to a file using preorder algorithm
void RecursivePreOrder(struct TreeNode* InputNode, FILE *OutputFile, int iPrintToOutputFile)
{
